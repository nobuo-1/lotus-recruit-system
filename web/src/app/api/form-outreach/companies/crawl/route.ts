...
function parseSearchHtml(html: string): RawRow[] {
  ...
  while ((m = linkRe.exec(html))) {
    ...
    const name = (candNames.find(Boolean) || null)?.trim() || null;
...
/** ★追加：住所キーワードでNTA検索結果ページを叩く（フォールバック方式） */
 ...
 * 出力: { inserted_new, tried, keywords_used, rows_preview, dropped_null_name, preview_null_names, using_service_role, trace, error?, hint? }
 */
export async function POST(req: Request) {
  ...
  try {
    ...
    // 名称欠落レコードに対して詳細補完を最大 200 件まで
    const needFill = deduped.filter((r) => !r.name).slice(0, DETAIL_FILL_CAP);
    ...
    // company_name NOT NULL 対応
    const withName = deduped.filter(
      (r) => !!(r.name && r.name.trim().length > 0)
    );
    const droppedNullName = deduped.length - withName.length;
    const previewNullNames = deduped
      .filter((r) => !r.name)
      .slice(0, 40)
    ...
    if (dryRun) {
      return NextResponse.json({
        ...
        keywords_used: keywordsUsed.slice(0, 40),
        rows_preview,
        dropped_null_name: droppedNullName,
        preview_null_names: previewNullNames,
        trace,
    ...
    const toSave = withName.slice(0, Math.max(want * 3, want));
    ...
    const payload = toSave.map((r) => ({
      ...
      company_name: r.name!.trim(), // ← 必ず非 null・非空
      address: r.address ?? null,
      detail_url: r.detail_url ?? null,
      source: "nta-crawl",
      scraped_at: now,
